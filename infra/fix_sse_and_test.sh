...
echo "🔧 Regravando apps/api/src/index.js (com SSE, limpo)..."
...
echo "🧱 Rebuild apenas da API (sem cache) ..."
docker compose build --no-cache api >/dev/null
docker compose up -d api >/dev/null

echo "🔍 Testando SSE direto na API (porta 3001)..."
curl -s -iN "http://localhost:3001/api/devices/stream?projectId=proj1" --max-time 3 | sed -n '1,12p'

echo "🔍 Testando SSE via Caddy (porta 80)..."
curl -s -iN "http://localhost/api/devices/stream?projectId=proj1" --max-time 3 | sed -n '1,12p'
